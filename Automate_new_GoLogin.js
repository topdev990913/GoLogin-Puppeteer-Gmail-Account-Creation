...
async function getURL() {
    username = 'akadoma9827@gmail.com';
    password = '123mehmet';
    ////Open Target URL////
    try {
        const response_gologin = await axios.post('https://api.gologin.com/api/v1/auth/login', {
            username,
            password
        });
        if (response.data.success) {
            ...
            // Step 2: Launch Puppeteer and connect it to the GoLogin browser
            const browser = await puppeteer.connect({
                browserWSEndpoint: `wss://api.gologin.com/api/v1/ws/${profileId}`,
                headless: false
            });
            ...
            await browser.close();
                ...
                throw new Error(response_gologin.data.message);
              }
        } catch (error) {
            getURL();
        }
